...
/// 大綱項目協議，提供子項目的存取能力
public protocol OutlineItem: Identifiable, Hashable {
  /// 子項目陣列，如果沒有子項目則為nil
...
public struct DraggableOutlineView<Item: OutlineItem>: View {
  // 數據
  ...
  private func removeItemFromTree(source: Item, items: inout [Item]) -> Bool {
    ...
    for i in 0 ..< items.count {
      guard var children = items[i].children else { continue }
